 ...
 * Uses get_cfo_decision_intelligence RPC for runway, prioritization, alerts, actions.
 */
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-sky-300">{formatBase(runway.daily_burn)}</p>
                <p className="text-xs text-slate-500">30-day average</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-1">Open Payables</p>
                <p className="text-xl font-semibold text-amber-300">{formatBase(runway.pending_payable)}</p>
              </div>
              <div>
                    ...
                    <div key={v.supplier_id}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="text-right">
                          <span className="text-sm font-semibold text-amber-300">{formatBase(v.total_exposure)}</span>
                          {v.overdue_amount > 0 && (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <Badge variant="outline" className="text-xs border-slate-600 text-slate-300">
                            {formatBase(action.details.amount || action.details.overdue_amount || 0)}
                          </Badge>
...
function formatAlertMessage(alert: Alert): string {
  switch (alert.alert_type) {
    case 'high_outflow_week':
      return `₹${formatCompactNum(alert.details.due_7d)} payable in next 7 days — ${alert.details.multiplier}x your average weekly burn`;
    case 'low_runway':
...
function formatActionTitle(action: SuggestedAction): string {
  const vendorId = action.details.vendor_id || '';
  const label = `PS-${vendorId.substring(0, 6).toUpperCase()}`;
  switch (action.action_type) {
    case 'delay_payment':
      return `Delay ${label} by ${action.details.suggested_delay_days}d → frees ${formatBase(action.details.amount)}`;
...
function formatCompactNum(val: number): string {
  if (val >= 10000000) return `${(val / 10000000).toFixed(1)} Cr`;
  if (val >= 100000) return `${(val / 100000).toFixed(1)} L`;
  return Math.round(val).toLocaleString('en-IN');
}
