...
import InviteTeamCTA from '@/components/dashboard/InviteTeamCTA';
        ...
        {isBuyerRole && activeManagementView ? (
          ...
          <div className="mb-3 sm:mb-8">
            ...
            <p className="text-xs sm:text-base text-muted-foreground truncate">
              {selectedPurchaser
                ? <>{(liveProfile?.company_name || user?.user_metadata?.company_name || '').split('·')[0].trim()} • {selectedPurchaser.display_name}{selectedPurchaser.email ? ` (${selectedPurchaser.email})` : ''} • {(selectedPurchaser.role || '').toUpperCase()}</>
                : <>{(liveProfile?.company_name || user?.user_metadata?.company_name || '').split('·')[0].trim()} • {role?.toUpperCase()}</>}
            </p>
          </div>
        ) : (
          <div className="mb-3 sm:mb-8">
