-- Fix scoped RPCs so NULL p_selected_purchaser = company-wide view (for management).
-- Self-only roles still hard-pinned to themselves.
...
BEGIN
  ...
  IF v_scope.is_self_only THEN
    -- Purchasers always pinned to self
    v_effective_purchaser := p_user_id;
  ELSIF v_scope.is_management THEN
    -- Management: NULL = company-wide; else view-as that purchaser
...
BEGIN
  ...
  -- Audit only when impersonating a specific purchaser
  IF NOT v_company_wide AND v_effective_purchaser <> p_user_id AND NOT v_scope.is_self_only THEN
    BEGIN
