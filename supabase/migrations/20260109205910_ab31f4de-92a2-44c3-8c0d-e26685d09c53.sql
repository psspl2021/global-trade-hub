...
-- ============================================
-- FIX GAPS 1, 2, 3 in Supplier Selection Functions
-- ============================================
...
BEGIN
  ...
  -- Select best bid with capacity check
  FOR v_all_bids IN
    SELECT 
      b.id as bid_id,
      b.supplier_id,
  ...
  -- GAP 1 FIX: Lock supplier row before capacity increment
  PERFORM 1
  ...
  -- GAP 2 FIX: Mark all other pending bids as rejected
  UPDATE public.bids
...
BEGIN
  ...
  -- GAP 3 FIX: Add delivery/quality thresholds in WHERE clause
...
-- Insert default risk thresholds config if not exists
INSERT INTO public.procurement_config (config_key, config_value, description)
VALUES (
  'risk_thresholds',
  '{"min_delivery_rate": 0.6, "max_quality_risk": 0.7}'::JSONB,
  'Minimum delivery rate and maximum quality risk for auto-assignment eligibility'
)
