...
BEGIN
  ...
  -- Summary aggregation (existing behaviour)
  SELECT
    CASE
  ...
  -- Decision intelligence insights — CEO / CFO only
  IF v_role IN ('ceo', 'cfo') THEN
    ...
    FROM (
      SELECT
        COALESCE(po.supplier_name, 'Unknown') AS supplier_name,
        SUM(po.po_value_base_currency) AS supplier_value
      FROM purchase_orders po
      WHERE po.buyer_company_id = ANY(v_company_ids)
      GROUP BY COALESCE(po.supplier_name, 'Unknown')
