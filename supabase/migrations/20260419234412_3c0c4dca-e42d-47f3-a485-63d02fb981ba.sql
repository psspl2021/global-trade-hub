-- Reconcile Decision Intelligence with PO data:
-- compute overdue %, avg delay, supplier concentration, cash pressure,
-- priority, risk level, upcoming payments, root causes, and actions
-- from the same scoped purchase orders the cards already use.
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      po.supplier_id, po.product_category,
  ...
  -- Purchaser accountability via join table
  WITH scoped AS (
    SELECT
      po.id AS po_id,
  ...
  -- Supplier concentration (top supplier share of total payable)
  WITH scoped AS (
    SELECT
      po.supplier_id,
      po.po_value_base_currency,
  ...
  -- Root causes (only when sample is statistically meaningful)
  IF v_overdue_count >= 3 THEN
    WITH overdue_pos AS (
      SELECT
        po.id, po.supplier_id, po.product_category, po.po_value_base_currency
      FROM public.get_scoped_purchase_orders(p_user_id) po
      WHERE po.payment_due_date IS NOT NULL
        AND po.payment_due_date < CURRENT_DATE
        AND COALESCE(po.payment_status,'') <> 'paid'
    ...
    by_category AS (
      SELECT
        COALESCE(o.product_category,'uncategorized') AS id,
        COALESCE(o.product_category,'Uncategorized') AS name,
        SUM(o.po_value_base_currency) AS amount,
        COUNT(*)::int AS count
      FROM overdue_pos o
  ...
  -- Recommended actions (deterministic, threshold-driven)
  WITH a AS (
    SELECT * FROM (VALUES
