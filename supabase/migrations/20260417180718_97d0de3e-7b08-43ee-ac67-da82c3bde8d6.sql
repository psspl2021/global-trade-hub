...
BEGIN
  ...
  -- Root cause: purchasers (deterministic ordering with tiebreaker)
  ...
  -- Root cause: suppliers (deterministic ordering with tiebreaker)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', agg.supplier_id,
  ...
  -- Numerical contract: ALL ratios returned as percentages (0-100) for UI consistency
  ...
  -- Note: temp table _scoped_pos auto-drops at transaction end (ON COMMIT DROP).
  -- Explicit DROP removed to avoid edge-case errors on retry.
