...
BEGIN
  -- Resolve role + company scope (single source of truth)
  ...
  -- Compute internally as ratio (0-1); presentation layer uses pct (0-100).
  -- Threshold logic (priority/risk_level) MUST remain ratio-based to prevent drift.
  ...
  FROM (
    ...
    ORDER BY v DESC NULLS LAST, supplier_id
    LIMIT 1
  ) t
  ...
  FROM (
    ...
    ORDER BY amount DESC NULLS LAST, po.created_by
    LIMIT 3
  ) agg
  ...
  FROM (
    ...
    ORDER BY amount DESC NULLS LAST, po.supplier_id
    LIMIT 3
  ) agg
  ...
  -- NUMERICAL CONTRACT
  --   overdue_ratio (0-1)   : LEGACY — retained for backward compatibility, will be deprecated.
  --   overdue_pct   (0-100) : PREFERRED — use this for all new UI / API consumers.
  v_insights := jsonb_build_object(
    ...
    'priority', CASE
      WHEN v_overdue_ratio >= 0.3 THEN 'CRITICAL'
      WHEN v_overdue_ratio >= 0.1 THEN 'WARNING'
      ELSE 'STABLE'
    END,
