...
BEGIN
  -- Explicit early role + scope resolution (single source of truth for control plane)
  ...
  -- NO_ROLE: authorization failure (user has no active company membership)
  ...
  IF v_overdue > 0 THEN
    v_actions := v_actions || jsonb_build_array(jsonb_build_object(
      'type', 'CLEAR_OVERDUE', 'title', 'Clear overdue payables',
  ...
  IF v_top_supplier_share >= 60 THEN
    v_actions := v_actions || jsonb_build_array(jsonb_build_object(
      'type', 'DIVERSIFY_SUPPLIERS', 'title', 'Reduce supplier dependency',
      'description', 'Single supplier concentration risk detected',
      'impact', v_top_supplier_share));
  END IF;
  IF v_payable_7d > 0 THEN
    v_actions := v_actions || jsonb_build_array(jsonb_build_object(
      'type', 'PLAN_CASHFLOW', 'title', 'Plan upcoming cashflow',
