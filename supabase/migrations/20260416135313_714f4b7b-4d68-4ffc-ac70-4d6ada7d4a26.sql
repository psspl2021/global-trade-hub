...
BEGIN
  ...
  IF v_company_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'actions','[]'::jsonb,'alerts','[]'::jsonb,'headline','No financial data available','trends',jsonb_build_object('burn_7d_vs_prev_pct',0,'overdue_change_pct',0,'payable_growth_pct',0),'system_confidence',0
    );
  END IF;

  -- ══════════════════════════════════════════
  -- BASE CTEs via direct queries (single source)
  -- ══════════════════════════════════════════

  -- Total payable (currency-normalized)
  ...
  -- Due in 7 days (using payment_due_date with fallback)
  ...
  -- Overdue (financial: payment_due_date, not delivery date)
  SELECT COALESCE(SUM(po.po_value_base_currency), 0),
  ...
  -- Burn 30d (currency-normalized from payment audit logs)
  SELECT COALESCE(SUM(
    CASE
  ...
  -- Clearance days
  v_clearance_days := CASE WHEN v_avg_daily_burn > 0 THEN CEIL(v_total_payable / v_avg_daily_burn) ELSE 0 END;
  ...
  -- Payable previous (7 days ago approximation - exclude recently closed)
  SELECT COALESCE(SUM(po.po_value_base_currency), 0)
  INTO v_payable_prev
  FROM purchase_orders po
  WHERE po.buyer_company_id = v_company_id
  ...
  -- ══════════════════════════════════════════
  -- BURN MULTIPLIER (clean: 7d payable vs avg weekly burn)
  -- ══════════════════════════════════════════
  v_burn_multiplier := CASE
    WHEN v_burn_30d > 0 THEN ROUND(v_payable_7d / (v_burn_30d / 4.0), 1)
    ELSE 0
  END;
  ...
  v_payable_severity := CASE
    WHEN v_clearance_days > 45 THEN 'critical'
    WHEN v_clearance_days > 30 THEN 'high'
    WHEN v_clearance_days > 14 THEN 'moderate'
    ELSE 'normal'
  END;
  ...
  v_clearance_impact_days := CASE
    WHEN v_avg_daily_burn > 0 THEN CEIL(v_payable_7d / v_avg_daily_burn)
    ELSE 0
  END;

  -- ══════════════════════════════════════════
  -- HEADLINE (severity-first, with K/L formatting)
  -- ══════════════════════════════════════════
  IF v_has_overdue AND v_overdue_worst_days >= 7 THEN
    v_formatted_amount := CASE
      WHEN v_overdue_amount >= 100000 THEN ROUND(v_overdue_amount / 100000, 1) || 'L'
      ELSE ROUND(v_overdue_amount / 1000) || 'K'
    END;
  ...
  ELSIF v_has_high_outflow THEN
    v_formatted_amount := CASE
      WHEN v_payable_7d >= 100000 THEN ROUND(v_payable_7d / 100000, 1) || 'L'
      ELSE ROUND(v_payable_7d / 1000) || 'K'
    END;
  ...
  ELSIF v_total_payable > 0 THEN
    v_formatted_amount := CASE
      WHEN v_total_payable >= 100000 THEN ROUND(v_total_payable / 100000, 1) || 'L'
      ELSE ROUND(v_total_payable / 1000) || 'K'
    END;
  ...
  IF v_has_overdue THEN
    ...
    v_actions := v_actions || jsonb_build_array(jsonb_build_object(
      ...
      'impact', '+' || CASE WHEN v_avg_daily_burn > 0 THEN CEIL(v_overdue_amount / v_avg_daily_burn) ELSE 0 END || ' days clearance improvement',
    ...
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object('type','overdue_risk','severity',v_overdue_severity,'metric',v_overdue_worst_days,'message',v_overdue_worst_days||' days overdue — '||v_overdue_vendor_count||' vendor(s) affected'));
  END IF;

  -- High outflow action (suppress burn reduction if overdue exists)
  IF v_has_high_outflow THEN
    ...
    END IF;
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object('type','high_outflow','severity',v_due7_severity,'metric',v_burn_multiplier,'message',v_burn_multiplier||'x burn this week'));
  END IF;
  ...
  IF v_concentration_risk THEN
    ...
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object('type','vendor_concentration','severity','high','metric',v_top_vendor_share,'message',ROUND(v_top_vendor_share)||'% concentrated on '||v_top_vendor));
  END IF;

  -- Sort actions by priority_score descending (deterministic)
  SELECT COALESCE(jsonb_agg(a ORDER BY (a->>'priority_score')::int DESC), '[]'::jsonb)
  INTO v_actions
  FROM jsonb_array_elements(v_actions) a;

  -- ══════════════════════════════════════════
  -- CONFIDENCE (composite, data-driven)
  -- ══════════════════════════════════════════
  ...
  v_result := jsonb_build_object(
    ...
    'trends', jsonb_build_object(
      'burn_7d_vs_prev_pct', CASE WHEN v_burn_prev > 0 THEN ROUND(((v_burn_30d - v_burn_prev) / v_burn_prev) * 100, 1) ELSE 0 END,
      'overdue_change_pct', CASE WHEN v_overdue_prev > 0 THEN ROUND(((v_overdue_amount - v_overdue_prev) / v_overdue_prev) * 100, 1) ELSE 0 END,
      'payable_growth_pct', CASE WHEN v_payable_prev > 0 THEN ROUND(((v_total_payable - v_payable_prev) / v_payable_prev) * 100, 1) ELSE 0 END
    ),
