...
BEGIN
  -- Materialize scoped dataset ONCE per call
  ...
  -- Root cause: purchasers (internal users — prefer contact_person/full_name fallback)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', agg.purchaser_id,
    'name', COALESCE(NULLIF(pr.contact_person, ''), LEFT(agg.purchaser_id::text,8), 'Unknown'),
    'amount', agg.amount,
