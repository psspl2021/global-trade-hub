...
DECLARE
  ...
  v_overdue_ratio numeric := 0;  -- 0-1 (legacy)
  v_overdue_pct numeric := 0;    -- 0-100 (new contract)
BEGIN
  ...
  -- Numerical contract:
  --   overdue_ratio: 0-1 (LEGACY — retained for backward compatibility)
  --   overdue_pct:   0-100 (NEW — preferred for UI consumers)
  v_insights := jsonb_build_object(
    'overdue_ratio', ROUND(v_overdue_ratio, 4),
