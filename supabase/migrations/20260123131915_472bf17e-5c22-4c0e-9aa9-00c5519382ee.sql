-- ISSUE 3 FIX: Update signal_source constraint to include 'rfq_award'
ALTER TABLE public.demand_intelligence_signals
DROP CONSTRAINT IF EXISTS demand_intelligence_signals_signal_source_check;

ALTER TABLE public.demand_intelligence_signals
ADD CONSTRAINT demand_intelligence_signals_signal_source_check
CHECK (
  signal_source IN (
    'keyword_scan',
    'signal_page',
...
-- PRODUCTION-SAFE AWARD FUNCTION with all fixes
...
BEGIN
  ------------------------------------------------------------------
  -- SAFETY 1: Prevent duplicate activation for same award
  ------------------------------------------------------------------
  IF EXISTS (
    SELECT 1
    FROM public.demand_intelligence_signals
    WHERE converted_to_rfq_id = req_id
      AND awarded_bid_id = bid_id
      AND lane_state = 'activated'
  ) THEN
  ...
  ------------------------------------------------------------------
  -- SAFETY 2: Get bid details with FOR UPDATE lock (prevents race)
  ------------------------------------------------------------------
  SELECT b.id,
         b.supplier_id,
         ...
         COALESCE(
           (SELECT SUM(bi.quantity) FROM public.bid_items bi WHERE bi.bid_id = b.id),
           req_record.quantity
         ) AS total_qty
  INTO bid_record
  FROM public.bids b
  ...
  ------------------------------------------------------------------
  -- Create activated demand intelligence lane
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  -- Find matching supplier capacity lane
  ------------------------------------------------------------------
  SELECT *
  INTO capacity_lane
