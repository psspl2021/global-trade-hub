...
-- Add logistics-aware logic to calculate_price_confidence function
...
BEGIN
  ...
  IF p_bid_id IS NOT NULL THEN
    ...
    -- Get logistics cost from bid service_fee or related logistics bid
    IF v_bid IS NOT NULL THEN
      v_logistics_cost := COALESCE(v_bid.service_fee, 0);
    END IF;
  END IF;

  -- Fetch market index for category
  ...
  IF v_market IS NOT NULL AND v_sample_size > 0 THEN
  ...
  -- If no market data, use conservative defaults
  IF v_market IS NULL THEN
    v_market := ROW(
      ...
      COALESCE(v_avg_price, 0),
      COALESCE(v_avg_price, 0),
      0.5, -- volatility_index (moderate)
      0.5, -- demand_index
      0.5, -- supply_index
      0, -- price_std_deviation
      NOW(),
      NOW(),
      NULL,
      0
  ...
  IF v_sample_size < 5 THEN
    v_confidence_suppressed := TRUE;
    v_market.volatility_index := 0.6; -- Conservative volatility
  END IF;

  -- MODE A: Bidding mode with actual bid
  IF v_normalized_mode = 'bidding' AND v_bid IS NOT NULL THEN
    -- Count total bids for this requirement
    SELECT COUNT(*) INTO v_total_bids
    FROM bids
    WHERE requirement_id = p_requirement_id
      AND status IN ('submitted', 'accepted');

    -- Price position: how does this price compare to market average?
    IF v_market.average_market_price > 0 THEN
      v_price_position := 1 - ABS(v_bid.buyer_visible_price - v_market.average_market_price) 
                          / v_market.average_market_price;
      v_price_position := GREATEST(0, LEAST(1, v_price_position));
    END IF;

    -- Market stability based on volatility
    v_market_stability := 1 - COALESCE(v_market.volatility_index, 0.5);

    -- Competition score based on number of bids
    ...
    IF v_market.max_market_price > v_market.min_market_price THEN
      v_price_spread_ratio := 1 - (v_market.max_market_price - v_market.min_market_price) 
                              / v_market.average_market_price;
  ...
  ELSE
    -- Use historical variance for confidence
    ...
    -- Conservative scoring for auto-assign
    v_price_position := 0.7; -- Assumed alignment
    v_market_stability := 1 - COALESCE(v_market.volatility_index, 0.5);
    v_competition_score := LEAST(0.7, 1 - v_historical_variance); -- Capped for Mode B
  ...
  -- Generate buyer-safe messaging
  v_buyer_message := CASE v_confidence_label
    WHEN 'HIGH' THEN 'This price is well-aligned with current market conditions and verified supplier availability.'
    WHEN 'MEDIUM' THEN 'The price reflects current market dynamics. Prices may vary based on demand and availability.'
    ELSE 'Market conditions are variable. We recommend reviewing multiple options for best value.'
  END;

  -- Price behavior note
  v_price_behavior_note := CASE
    WHEN v_market.volatility_index > 0.7 THEN 'Prices in this category are currently volatile. Consider locking in pricing early.'
    WHEN v_market.demand_index > v_market.supply_index THEN 'High demand may affect pricing. Early procurement recommended.'
  ...
  IF p_bid_id IS NOT NULL THEN
    -- Upsert for records WITH bid_id
    INSERT INTO price_confidence_scores (
      ...
      p_requirement_id,
      p_bid_id,
      COALESCE(v_bid.buyer_visible_price, v_avg_price, v_market.average_market_price, 1),
      v_confidence_score,
      v_confidence_label,
      v_buyer_message,
    ...
    DO UPDATE SET
      ...
      created_at = NOW();
  ELSE
    -- Upsert for records WITHOUT bid_id
    INSERT INTO price_confidence_scores (
      requirement_id,
      bid_id,
      buyer_visible_price,
      ...
      p_requirement_id,
      NULL,
      COALESCE(p_buyer_visible_price, v_avg_price, v_market.average_market_price, 1),
      v_confidence_score,
      v_confidence_label,
      v_buyer_message,
      v_price_behavior_note,
