-- =============================================================
-- FIX 1 + 2: escalate_overdue_override_acks
--   - Fix notification type 'po_flagged_stale'
--   - Use correct TABLE return pattern for get_po_approver
-- =============================================================
...
BEGIN
  ...
  -- ===========================================================
  -- C) FLAGGED STALE (FIXED TYPE: po_flagged_stale)
  -- ===========================================================
  FOR v_row IN
...
-- =============================================================
-- FIX 3: Lifecycle stage precedence in get_company_intelligence_v2
--   Correct order: PAID > FINALIZED > FORCE_CLOSED > FLAGGED >
--                  PENDING_ACK > CEO_OVERRIDE > PENDING_APPROVAL > PO_CREATED
-- =============================================================
...
BEGIN
  ...
  -- Enriched dataset with deterministic lifecycle stage
  WITH base AS (
    SELECT
      po.id,
      po.po_number,
      ...
      -- Correct precedence:
      -- PAID > FINALIZED > FORCE_CLOSED > FLAGGED > PENDING_ACK > CEO_OVERRIDE > PENDING_APPROVAL > PO_CREATED
      CASE
        ...
        WHEN po.approval_status = 'pending_ack' THEN 'PENDING_ACK'
        WHEN po.ceo_override = true THEN 'CEO_OVERRIDE'
        WHEN po.approval_status = 'pending_approval' THEN 'PENDING_APPROVAL'
        ELSE 'PO_CREATED'
  ...
  v_insights := jsonb_build_object(
    ...
    'note', 'Lifecycle precedence: PAID > FINALIZED > FORCE_CLOSED > FLAGGED > PENDING_ACK > CEO_OVERRIDE > PENDING_APPROVAL > PO_CREATED'
  );
