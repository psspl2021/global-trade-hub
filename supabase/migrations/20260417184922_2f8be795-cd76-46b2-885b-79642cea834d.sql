...
DECLARE
  ...
  v_min_sample int := 3;  -- statistical floor for root-cause attribution
BEGIN
  ...
  IF v_role IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'upcoming_payments', '[]'::jsonb, 'empty', true
    );
  END IF;
  ...
  -- Top supplier: prefer profile name, then PO vendor_name, then UUID stub
  SELECT
    COALESCE(
  ...
  -- Root cause: only attribute when sample is statistically meaningful (n >= 3)
