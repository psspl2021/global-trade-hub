...
-- STEP 1.1: Harden CHECK constraint (idempotent)
...
-- STEP 1.2: Forensic trigger lock (idempotent)
...
BEGIN
  IF NEW.decision_action IS NOT NULL
    ...
    RAISE EXCEPTION
      'Invalid decision_action %. Activation MUST go via activate_demand_lane()',
      NEW.decision_action;
  END IF;
  RETURN NEW;
END;
...
-- STEP 2: Canonical RPC (final version)
...
BEGIN
  ...
  INSERT INTO lane_events (
    signal_id, event_type, country, category,
    from_state, to_state, actor, occurred_at
  ) VALUES (
    p_signal_id, 'LANE_ACTIVATED',
    sig.country, sig.category,
    COALESCE(sig.lane_state,'detected'), 'activated',
    'admin', NOW()
  );
...
-- STEP 3: Drop legacy RPC
DROP FUNCTION IF EXISTS activate_lane_from_signal(text, text) CASCADE;
DROP FUNCTION IF EXISTS activate_lane_from_signal(text, text, uuid) CASCADE;

-- STEP 4: Revoke direct UPDATE from anon/authenticated
REVOKE UPDATE ON demand_intelligence_signals FROM anon, authenticated;
