...
BEGIN
  ...
  IF v_role IN ('ceo', 'cfo') THEN
    ...
    -- Top supplier concentration (resolved via profiles.full_name)
    SELECT supplier_name, supplier_value
    INTO v_top_supplier_name, v_top_supplier_value
    FROM (
      SELECT
        COALESCE(p.full_name, LEFT(po.supplier_id::text, 8), 'Unknown') AS supplier_name,
        SUM(po.po_value_base_currency) AS supplier_value
      FROM purchase_orders po
      LEFT JOIN profiles p ON p.id = po.supplier_id
      WHERE po.buyer_company_id = ANY(v_company_ids)
      GROUP BY COALESCE(p.full_name, LEFT(po.supplier_id::text, 8), 'Unknown')
    ...
    -- Upcoming payments (resolved via profiles.full_name)
