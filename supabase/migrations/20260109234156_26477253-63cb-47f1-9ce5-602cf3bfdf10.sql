...
-- Add defensive clamping and softer Mode B market pressure penalty
...
BEGIN
  ...
  -- Calculate market pressure (demand/supply imbalance)
  ...
  IF v_normalized_mode = 'bidding' AND v_bid IS NOT NULL THEN
    ...
    -- Apply demand/supply imbalance penalty (max 10% reduction for bidding)
  ...
  ELSE
    ...
    -- Apply softer demand/supply imbalance penalty for auto-assign (max 5% reduction)
    v_confidence_score := v_confidence_score * (1 - v_market_pressure * 0.05);
  ...
  -- Defensive clamp to valid range (0-100)
