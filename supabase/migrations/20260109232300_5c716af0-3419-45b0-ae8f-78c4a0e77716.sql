...
-- Fix unsafe ROW() cast and enhance buyer messaging with logistics clarity
...
DECLARE
  ...
  -- Explicit market variables to avoid unsafe ROW() cast
...
BEGIN
  ...
  IF p_bid_id IS NOT NULL THEN
    ...
    -- Get logistics cost from bid service_fee
  ...
  IF v_sample_size < 5 THEN
    v_confidence_suppressed := TRUE;
    v_market_volatility := 0.6; -- Conservative volatility
  END IF;
