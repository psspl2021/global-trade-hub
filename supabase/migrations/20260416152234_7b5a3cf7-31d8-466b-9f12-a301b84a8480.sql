...
BEGIN
  ...
  INTO v_top_vendor_id, v_top_vendor_name, v_top_vendor_amount, v_total_vendors
  FROM purchase_orders po
  WHERE po.buyer_company_id = p_company_id
    AND po.status IN ('approved','dispatched','delivered')
  ...
  v_health_score := GREATEST(0, LEAST(100,
    ...
    - CASE WHEN v_burn_prev >= 10000 AND v_burn_30d > v_burn_prev AND ((v_burn_30d - v_burn_prev) / v_burn_prev) > 0.2 THEN 10 ELSE 0 END
  ));

  -- Pressure score (raw stress intensity)
  v_pressure_score := ROUND(
  ...
  -- Confidence
  v_confidence := CASE
    WHEN v_burn_30d > 0 AND v_total_payable > 0 AND v_total_vendors > 0 THEN 0.9
    WHEN v_total_payable > 0 THEN 0.6
    ELSE 0.3
  END;

  -- Headline (severity-first)
  IF v_payable_severity = 'critical' THEN
  ...
  ELSIF v_payable_severity = 'high' THEN
    v_headline := format('🟠 %s exposure with elevated risk signals',
  ...
  ELSIF v_total_payable > 0 THEN
    v_headline := format('🟢 %s total payable — operations stable',
      CASE WHEN v_total_payable >= 100000 THEN format('₹%sL', ROUND(v_total_payable/100000, 1))
           WHEN v_total_payable >= 1000 THEN format('₹%sK', ROUND(v_total_payable/1000, 0))
           ELSE format('₹%s', ROUND(v_total_payable, 0)) END);
  ELSE
  ...
  -- Trends (with semantic impact direction)
  IF v_burn_prev >= 10000 THEN
    v_trends := v_trends || jsonb_build_object(
      ...
      'direction', CASE WHEN v_burn_30d >= v_burn_prev THEN 'up' ELSE 'down' END,
      'impact', CASE WHEN v_burn_30d > v_burn_prev THEN 'negative' ELSE 'positive' END
    );
  END IF;
  ...
  IF v_prev_overdue >= 10000 THEN
    v_trends := v_trends || jsonb_build_object(
      ...
      'direction', CASE WHEN v_overdue_amount >= v_prev_overdue THEN 'up' ELSE 'down' END,
      'impact', CASE WHEN v_overdue_amount > v_prev_overdue THEN 'negative' ELSE 'positive' END
    );
  END IF;
  ...
  IF v_prev_payable >= 10000 THEN
    v_trends := v_trends || jsonb_build_object(
      ...
      'direction', CASE WHEN v_total_payable >= v_prev_payable THEN 'up' ELSE 'down' END,
      'impact', CASE WHEN v_total_payable > v_prev_payable THEN 'negative' ELSE 'positive' END
    );
  END IF;
  ...
  SELECT COALESCE(jsonb_agg(a ORDER BY (a->>'priority')::int DESC), '[]'::jsonb)
  INTO v_actions
