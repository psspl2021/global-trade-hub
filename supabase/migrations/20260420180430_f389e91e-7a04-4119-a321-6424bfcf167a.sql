...
BEGIN
  ...
  agg AS (
    SELECT
      ...
      ) ORDER BY created_at DESC), '[]'::jsonb) AS pos_json,
      COUNT(*) AS po_count,
      COALESCE(SUM(po_value_base_currency), 0) AS total_payable,
      COALESCE(SUM(po_value_base_currency) FILTER (WHERE is_payable), 0) AS overdue_sum_unused,
      COALESCE(SUM(po_value_base_currency) FILTER (WHERE due_7d), 0) AS payable_7d,
      COUNT(*) FILTER (WHERE is_overdue) AS overdue_count,
      COALESCE(SUM(po_value_base_currency) FILTER (WHERE is_overdue), 0) AS overdue_value
    FROM base
  ),
  stages AS (
    SELECT COALESCE(jsonb_object_agg(lifecycle_stage, c), '{}'::jsonb) AS stage_counts
    FROM (SELECT lifecycle_stage, COUNT(*)::int AS c FROM base GROUP BY lifecycle_stage) s
  )
  SELECT a.pos_json, a.po_count, a.total_payable, a.overdue_value, a.payable_7d, a.overdue_count, st.stage_counts
  INTO v_pos, v_po_count, v_total_payable, v_overdue, v_payable_7d, v_overdue_count, v_stage_counts
  FROM agg a, stages st;

  -- Top purchasers accountability
  WITH base AS (
    SELECT
      po.id, po.po_value_base_currency,
      (po.payment_due_date IS NOT NULL
        AND po.payment_due_date < CURRENT_DATE
        AND COALESCE(po.payment_status,'') <> 'paid') AS is_overdue,
      po.ceo_override,
  ...
  per_purchaser AS (
    SELECT
      ...
      COALESCE(SUM(b.po_value_base_currency),0) AS total_value,
      COUNT(*) FILTER (WHERE b.is_overdue)::int AS overdue_count,
      COUNT(*) FILTER (WHERE b.stage IN ('PENDING_ACK','PENDING_APPROVAL','FLAGGED','CEO_OVERRIDE'))::int AS stuck_count,
      COUNT(*) FILTER (WHERE b.ceo_override IS TRUE)::int AS override_count
    FROM base b
    JOIN public.purchase_order_purchasers pop ON pop.po_id = b.id
    LEFT JOIN public.profiles pr ON pr.id = pop.user_id
    GROUP BY pop.user_id, pr.company_name, pr.contact_person
    ORDER BY SUM(b.po_value_base_currency) DESC NULLS LAST
    LIMIT 10
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'purchaser_id', user_id::text,
    'purchaser', purchaser,
    'total_pos', total_pos,
