...
DECLARE
  ...
  v_active_purchasers int := 0;
BEGIN
  -- Resolve role + company scope
  ...
  -- ============================================================
  -- SCOPED DATASET (single source of truth for role + company)
  -- Materialized into a TEMP TABLE so every downstream query
  -- reuses the same filtered rowset — no scope drift possible.
  -- ============================================================
  CREATE TEMP TABLE IF NOT EXISTS _scoped_pos
    ON COMMIT DROP
    AS SELECT * FROM purchase_orders WHERE false;
  TRUNCATE _scoped_pos;

  INSERT INTO _scoped_pos
  SELECT po.*
  ...
  -- Summary (role-aware shape)
  IF v_role = 'hr' THEN
    SELECT COUNT(*), COUNT(DISTINCT po.created_by)
    INTO v_po_count, v_active_purchasers
    FROM _scoped_pos po;

    v_summary := jsonb_build_object(
      'po_count', v_po_count,
      'active_purchasers', v_active_purchasers
    );
  ELSE
    SELECT
  ...
  -- CEO/CFO insights (reuse same scoped dataset)
  IF v_role IN ('ceo', 'cfo') THEN
    ...
    -- MERGE (not overwrite) into v_insights
