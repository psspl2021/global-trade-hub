...
-- CONTRACT_SUMMARIES: drop all existing policies
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN (
    SELECT policyname FROM pg_policies WHERE tablename = 'contract_summaries'
  ) LOOP
...
-- DEMAND_INTELLIGENCE_SIGNALS: drop all existing policies
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN (
    SELECT policyname FROM pg_policies WHERE tablename = 'demand_intelligence_signals'
  ) LOOP
