...
-- Fix lane_events insert: from_state must not be NULL
...
BEGIN
  ------------------------------------------------------------------
  -- SAFETY 1 — Idempotent: one bid = one lane
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  -- SAFETY 2 — Lock bid row
  ------------------------------------------------------------------
  SELECT b.id, b.status, b.supplier_id
  INTO bid_record
  FROM public.bids b
  WHERE b.id = bid_id
  FOR UPDATE;
  ...
  ------------------------------------------------------------------
  -- SAFETY 3 — Atomically mark bid accepted
  ------------------------------------------------------------------
  UPDATE public.bids
  SET status = 'accepted',
      awarded_at = now()
  ...
  IF NOT FOUND THEN
    ...
    RAISE EXCEPTION 'Bid already awarded or invalid state: %', bid_id;
  END IF;
  ...
  ------------------------------------------------------------------
  -- Reserve supplier capacity (if lane exists)
  ------------------------------------------------------------------
  ...
  IF capacity_lane_id IS NOT NULL THEN
    ...
    SET 
      ...
      capacity_utilization_at_award = CASE 
        ...
        THEN ((COALESCE(current_capacity, 0) + awarded_value) / 
              (SELECT max_capacity_value FROM public.supplier_capacity_lanes WHERE id = capacity_lane_id)) * 100
        ELSE 0 
      END
    WHERE id = lane_id;
  END IF;
  ...
  ------------------------------------------------------------------
  -- Log lane activation event (FIXED: from_state = 'detected')
  ------------------------------------------------------------------
