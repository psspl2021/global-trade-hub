...
SELECT
  ...
  COUNT(*) FILTER (WHERE sent_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Kolkata')) AS sent_today,
  COUNT(*) FILTER (WHERE conversion_after_nudge = true AND sent_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Kolkata')) AS converted_today,
  COUNT(*) FILTER (WHERE channel = 'whatsapp_failed' AND sent_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Kolkata')) AS failed_today,
  ROUND(
    (COUNT(*) FILTER (WHERE conversion_after_nudge = true)::numeric
