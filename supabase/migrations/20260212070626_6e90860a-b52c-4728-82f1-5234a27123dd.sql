-- Fix handle_new_user function to safely handle enum casts and field name mismatches
...
BEGIN
  ...
  -- Safely cast logistics_partner_type (can be null, empty, or invalid)
  BEGIN
  ...
  -- Get location from either 'location' or 'address' field (frontend sends 'location')
  v_location := COALESCE(
...
EXCEPTION WHEN unique_violation THEN
  -- Handle duplicate phone/email gracefully - still create the user
  -- Try without phone constraint
  BEGIN
    INSERT INTO public.profiles (id, company_name, contact_person, email, phone, referred_by_name, referred_by_phone, logistics_partner_type, address, gstin)
    VALUES (
      NEW.id,
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Last resort: just continue and let the user be created
    NULL;
  END;
  
  -- Still create role and subscription
  IF NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = NEW.id) THEN
    BEGIN
      INSERT INTO public.user_roles (user_id, role)
      VALUES (NEW.id, v_role);
