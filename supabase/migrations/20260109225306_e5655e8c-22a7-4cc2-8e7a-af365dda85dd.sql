-- IMPROVEMENT 1: Replace ugly COALESCE index with cleaner partial unique indexes
DROP INDEX IF EXISTS uq_price_confidence_unique;

CREATE UNIQUE INDEX IF NOT EXISTS uq_price_confidence_with_bid
ON price_confidence_scores (requirement_id, bid_id)
WHERE bid_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_price_confidence_without_bid
ON price_confidence_scores (requirement_id)
WHERE bid_id IS NULL;

-- IMPROVEMENT 2 & 3: Update function to write sample_size back and admin-gate internal payload
CREATE OR REPLACE FUNCTION public.calculate_price_confidence(
  p_requirement_id UUID,
  p_bid_id UUID DEFAULT NULL,
  p_selection_mode TEXT DEFAULT 'sealed_bidding'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    ...
    -- IMPROVEMENT 2: Update sample_size in market_price_indices
  ...
  -- Apply minimum sample protection (GAP 1 fix)
  ...
  -- Calculate market pressure from demand/supply (GAP 2 fix)
  v_market_pressure := ABS(COALESCE(v_market.demand_index, 0.5) - COALESCE(v_market.supply_index, 0.5));

  -- MODE A: Sealed Bidding (L1 Selection)
  IF p_selection_mode = 'sealed_bidding' THEN
  ...
  ELSE
    ...
    v_market_stability := 1 - COALESCE(v_market.volatility_index, 0.5);
    v_competition_score := 1 - COALESCE(v_historical_variance, 0.5);
  END IF;

  -- Calculate final confidence score with market pressure (GAP 2 fix)
  ...
  IF p_selection_mode = 'auto_assign' AND v_confidence_score > 85 THEN
  ...
  -- Build internal payload (IMPROVEMENT 3: admin-only gate)
  IF v_is_admin THEN
    v_internal := jsonb_build_object(
      ...
      'mode_cap_applied', (p_selection_mode = 'auto_assign' AND v_confidence_suppressed),
      'transparency_cap_applied', (COALESCE(v_market.volatility_index, 0.5) > 0.6 OR v_price_spread_ratio > 0.15)
    );
  ELSE
    -- Non-admins get NULL internal payload
    v_internal := NULL;
  END IF;

  -- Store confidence score (GAP 5: upsert for immutability)
  INSERT INTO price_confidence_scores (
    ...
    v_confidence_label,
    v_buyer_message,
    COALESCE(v_bid.buyer_visible_price, 0),
    p_selection_mode,
    v_price_position,
    v_market_stability,
    v_competition_score,
  ...
  ON CONFLICT ON CONSTRAINT price_confidence_scores_pkey
  DO UPDATE SET
