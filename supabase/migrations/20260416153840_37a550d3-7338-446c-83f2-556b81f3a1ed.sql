...
BEGIN
  ...
  -- ══════════════════════════════════════════
  -- BURN MULTIPLIER (capped at 5x for interpretability)
  -- ══════════════════════════════════════════
  v_burn_multiplier := CASE
    WHEN v_burn_30d >= 10000 THEN LEAST(5, ROUND(v_payable_7d / (v_burn_30d / 4.0), 1))
    ELSE 0
  END;
  ...
  ELSIF v_concentration_risk AND v_top_vendor_share >= 70 THEN
  ...
  v_result := jsonb_build_object(
    ...
    'headline', v_headline,
    'trends', jsonb_build_array(
      jsonb_build_object('metric','burn_30d_vs_prev_pct','value',v_burn_trend,'direction',CASE WHEN v_burn_trend > 0 THEN 'up' WHEN v_burn_trend < 0 THEN 'down' ELSE 'flat' END,'impact',CASE WHEN v_burn_trend > 0 THEN 'negative' ELSE 'positive' END),
      jsonb_build_object('metric','overdue_change_pct','value',v_overdue_trend,'direction',CASE WHEN v_overdue_trend > 0 THEN 'up' WHEN v_overdue_trend < 0 THEN 'down' ELSE 'flat' END,'impact',CASE WHEN v_overdue_trend > 0 THEN 'negative' ELSE 'positive' END),
      jsonb_build_object('metric','payable_growth_pct','value',v_payable_trend,'direction',CASE WHEN v_payable_trend > 0 THEN 'up' WHEN v_payable_trend < 0 THEN 'down' ELSE 'flat' END,'impact',CASE WHEN v_payable_trend > 0 THEN 'negative' ELSE 'positive' END)
    ),
