-- Add a small helper that resolves a supplier_id to a human name
-- by checking buyer_suppliers first, then profiles.
CREATE OR REPLACE FUNCTION public._resolve_supplier_name(p_supplier_id uuid)
RETURNS text
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    (SELECT NULLIF(TRIM(COALESCE(bs.company_name, bs.supplier_name)), '')
       FROM public.buyer_suppliers bs WHERE bs.id = p_supplier_id LIMIT 1),
    (SELECT NULLIF(TRIM(COALESCE(pr.company_name, pr.contact_person)), '')
       FROM public.profiles pr WHERE pr.id = p_supplier_id LIMIT 1),
    'Supplier ' || LEFT(p_supplier_id::text, 6)
  );
$$;
...
BEGIN
  WITH ranked AS (
    SELECT uca.role, uca.company_id,
      ROW_NUMBER() OVER (
        ORDER BY
          CASE LOWER(uca.role)
    ...
    FROM public.user_company_access uca WHERE uca.user_id = p_user_id
  ),
  picked AS (SELECT role FROM ranked WHERE rn = 1)
  SELECT p.role, ARRAY_AGG(r.company_id) INTO v_role, v_company_ids
  FROM picked p
  JOIN public.user_company_access r ON r.user_id = p_user_id AND r.role = p.role
  GROUP BY p.role;
  ...
  FROM public.buyer_companies bc WHERE bc.id = v_company_ids[1] LIMIT 1;

  WITH base AS (
    SELECT po.id, po.po_number, po.po_value_base_currency, po.payment_due_date,
      po.payment_status, po.created_at, po.created_by, po.ceo_override,
      po.supplier_id, po.spend_category,
      (po.payment_due_date IS NOT NULL AND po.payment_due_date < CURRENT_DATE
      ...
      public.get_po_lifecycle_stage(po.payment_status, po.manager_ack_at, po.approval_status, po.ceo_override) AS lifecycle_stage
    FROM public.get_scoped_purchase_orders(p_user_id) po
  )
  SELECT
    COALESCE(jsonb_agg(jsonb_build_object(
      'po_id', id, 'po_number', po_number, 'amount', po_value_base_currency,
      'due_date', payment_due_date, 'stage', lifecycle_stage,
      'purchasers', public.get_po_purchasers(id), 'creator_id', created_by,
  ...
  WITH base AS (
    SELECT public.get_po_lifecycle_stage(po.payment_status, po.manager_ack_at, po.approval_status, po.ceo_override) AS stage
    FROM public.get_scoped_purchase_orders(p_user_id) po
  )
  SELECT COALESCE(jsonb_object_agg(stage, cnt), '{}'::jsonb) INTO v_stage_counts
  FROM (SELECT stage, COUNT(*) AS cnt FROM base GROUP BY stage) s;

  WITH scoped AS (
    SELECT po.id AS po_id, po.po_value_base_currency, po.ceo_override,
      (po.payment_due_date IS NOT NULL AND po.payment_due_date < CURRENT_DATE
        AND COALESCE(po.payment_status,'') <> 'paid') AS is_overdue,
      public.get_po_lifecycle_stage(po.payment_status, po.manager_ack_at, po.approval_status, po.ceo_override) AS stage
    FROM public.get_scoped_purchase_orders(p_user_id) po
  ),
  linked AS (
    SELECT pop.user_id AS purchaser_id,
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::jsonb) INTO v_top_purchasers
  FROM (
    SELECT purchaser_id, purchaser,
    ...
    FROM linked GROUP BY purchaser_id, purchaser
    ORDER BY COALESCE(SUM(po_value_base_currency), 0) DESC LIMIT 10
  ) t;
  ...
  WITH scoped AS (
    SELECT po.supplier_id, po.po_value_base_currency,
      (COALESCE(po.payment_status,'') <> 'paid') AS is_payable
    FROM public.get_scoped_purchase_orders(p_user_id) po
  ...
  agg AS (
    SELECT s.supplier_id,
      public._resolve_supplier_name(s.supplier_id) AS supplier_name,
      SUM(s.po_value_base_currency) FILTER (WHERE s.is_payable) AS payable_value,
      SUM(s.po_value_base_currency) AS total_value
    FROM scoped s GROUP BY s.supplier_id
  ),
  top1 AS (
    SELECT * FROM agg ORDER BY COALESCE(payable_value, total_value) DESC NULLS LAST LIMIT 1
  )
  SELECT jsonb_build_object(
    ...
    'concentration_pct', CASE WHEN v_total_payable > 0 THEN
  ...
  WITH up AS (
    SELECT po.id AS po_id, po.po_number,
      po.po_value_base_currency AS amount, po.payment_due_date AS due_date,
      public._resolve_supplier_name(po.supplier_id) AS supplier_name
    FROM public.get_scoped_purchase_orders(p_user_id) po
    WHERE po.payment_due_date IS NOT NULL
      AND po.payment_due_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'
      AND COALESCE(po.payment_status,'') <> 'paid'
    ORDER BY po.payment_due_date ASC LIMIT 25
  )
  SELECT COALESCE(jsonb_agg(row_to_json(up)), '[]'::jsonb) INTO v_upcoming FROM up;

  IF v_overdue_count >= 3 THEN
    WITH overdue_pos AS (
      SELECT po.id, po.supplier_id, po.spend_category, po.po_value_base_currency
      FROM public.get_scoped_purchase_orders(p_user_id) po
      WHERE po.payment_due_date IS NOT NULL
    ...
    by_purchaser AS (
      SELECT pop.user_id::text AS id,
        COALESCE(pr.company_name, pr.contact_person, 'User ' || LEFT(pop.user_id::text,6)) AS name,
        SUM(o.po_value_base_currency) AS amount, COUNT(*)::int AS count
      FROM overdue_pos o
      JOIN public.purchase_order_purchasers pop ON pop.po_id = o.id
      LEFT JOIN public.profiles pr ON pr.id = pop.user_id
      GROUP BY pop.user_id, pr.company_name, pr.contact_person
      ORDER BY SUM(o.po_value_base_currency) DESC NULLS LAST LIMIT 5
    ),
    by_supplier AS (
      SELECT o.supplier_id::text AS id,
        public._resolve_supplier_name(o.supplier_id) AS name,
        SUM(o.po_value_base_currency) AS amount, COUNT(*)::int AS count
      FROM overdue_pos o WHERE o.supplier_id IS NOT NULL
      GROUP BY o.supplier_id
      ORDER BY SUM(o.po_value_base_currency) DESC NULLS LAST LIMIT 5
    ),
    by_category AS (
      SELECT COALESCE(o.spend_category,'uncategorized') AS id,
        COALESCE(o.spend_category,'Uncategorized') AS name,
        SUM(o.po_value_base_currency) AS amount, COUNT(*)::int AS count
      FROM overdue_pos o GROUP BY o.spend_category
      ORDER BY SUM(o.po_value_base_currency) DESC NULLS LAST LIMIT 5
    )
    SELECT jsonb_build_object(
      'sufficient_sample', true, 'sample_size', v_overdue_count, 'min_sample_required', 3,
      'purchasers', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', id, 'name', name, 'amount', amount, 'count', count,
          'share_pct', ROUND((amount / NULLIF(v_overdue_value,0)) * 100, 1))) FROM by_purchaser), '[]'::jsonb),
      'suppliers', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', id, 'name', name, 'amount', amount, 'count', count,
          'share_pct', ROUND((amount / NULLIF(v_overdue_value,0)) * 100, 1))) FROM by_supplier), '[]'::jsonb),
      'categories', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', id, 'name', name, 'amount', amount, 'count', count,
          'share_pct', ROUND((amount / NULLIF(v_overdue_value,0)) * 100, 1))) FROM by_category), '[]'::jsonb)
    ) INTO v_root_causes;
  END IF;
  ...
  v_summary := jsonb_build_object(
    'po_count', v_total_pos, 'total_payable', v_total_payable, 'overdue', v_overdue_value,
    'payable_7d', v_payable_7d, 'total_pos', v_total_pos, 'total_value', v_total_value,
    'overdue_count', v_overdue_count, 'overdue_value', v_overdue_value,
    'override_count', v_override_count, 'flagged_count', v_flagged_count,
    'pending_ack_count', v_pending_ack_count, 'paid_count', v_paid_count,
    'finalized_count', v_finalized_count, 'base_currency', v_base_currency,
    'stage_counts', v_stage_counts
  );

  v_insights := jsonb_build_object(
    'top_purchasers', v_top_purchasers, 'stage_counts', v_stage_counts,
    ...
    'priority', v_priority, 'risk_level', v_risk_level,
    'supplier_risk', v_supplier_risk, 'root_causes', v_root_causes,
    'note', 'Decision Intelligence reconciled with PO data'
  );

  RETURN jsonb_build_object(
    'role', v_role, 'company_ids', to_jsonb(v_company_ids), 'base_currency', v_base_currency,
    'summary', v_summary, 'insights', v_insights, 'pos', v_pos,
    'actions', v_actions, 'upcoming_payments', v_upcoming,
    'empty', (v_total_pos = 0)
  );
END;
$function$;