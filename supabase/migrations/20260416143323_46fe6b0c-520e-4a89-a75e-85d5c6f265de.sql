...
BEGIN
  ...
  ELSIF v_has_overdue OR v_burn_multiplier >= 2.0 THEN
    v_payable_severity := 'critical';
  ...
  IF v_concentration_risk THEN
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      ...
      'severity', CASE WHEN v_top_vendor_share >= 70 THEN 'critical' ELSE v_payable_severity END,
  ...
  IF v_has_overdue THEN
    v_actions := v_actions || jsonb_build_array(jsonb_build_object(
      ...
      'impact', format('+%s days runway if cleared', LEAST(v_overdue_worst_days, 30)),
      'category', 'overdue'
    ));
  END IF;
  ...
  v_trends := jsonb_build_array(
    jsonb_build_object('metric', 'burn_30d_vs_prev_pct',
      'value', CASE WHEN v_burn_prev > 0 THEN ROUND(((v_burn_30d - v_burn_prev) / v_burn_prev) * 100, 1) ELSE 0 END,
    ...
    jsonb_build_object('metric', 'overdue_change_pct',
      'value', CASE WHEN v_overdue_prev > 0 THEN ROUND(((v_overdue_amount - v_overdue_prev) / v_overdue_prev) * 100, 1) ELSE 0 END,
    ...
    jsonb_build_object('metric', 'payable_growth_pct',
      'value', CASE WHEN v_payable_prev > 0 THEN ROUND(((v_total_payable - v_payable_prev) / v_payable_prev) * 100, 1) ELSE 0 END,
  ...
  v_health_score := GREATEST(0, LEAST(100,
    100
    - LEAST(v_overdue_worst_days * 5, 40)
    - CASE WHEN v_burn_multiplier >= 2.0 THEN 25 WHEN v_burn_multiplier >= 1.5 THEN 15 ELSE 0 END
    - CASE WHEN v_top_vendor_share >= 70 THEN 20 WHEN v_top_vendor_share >= 50 THEN 10 ELSE 0 END
    - CASE WHEN v_burn_30d > v_burn_prev AND v_burn_prev > 0 AND ((v_burn_30d - v_burn_prev) / v_burn_prev) > 0.2 THEN 10 ELSE 0 END
  ));
