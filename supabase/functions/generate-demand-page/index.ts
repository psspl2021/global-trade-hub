...
serve(async (req) => {
  ...
  try {
    // Auth check — accept both user JWT and service role key
...
Keep all data specific to the Indian B2B procurement context. Use realistic prices in INR.`;
    ...
    // Reverse linking: add this new slug to related pages' related_slugs (parallel, deduped)
    if (relatedSlugs.length > 0) {
      await Promise.all(
        relatedSlugs.map(async (relSlug) => {
          ...
          const updated = Array.from(new Set([...existing, slug])).slice(-10);
          await adminSupabase
            .from("demand_generated")
